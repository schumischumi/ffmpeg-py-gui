...
def test_exposed_objects(modulelevel_internal_objects: list[griffe.Object | griffe.Alias]) -> None:
    """All public objects in the internal API are exposed under `ffmpeg_py_gui`."""
    not_exposed = [
        obj.path
        for obj in modulelevel_internal_objects
        if obj.name not in ffmpeg_py_gui.__all__ or not hasattr(ffmpeg_py_gui, obj.name)
    ]
...
def test_single_locations(public_api: griffe.Module) -> None:
    ...
    multiple_locations = {}
    for obj_name in ffmpeg_py_gui.__all__:
...
def test_inventory_matches_api(
    ...
    public_api_paths.add("ffmpeg_py_gui")
    for item in inventory.values():
        if (
            item.domain == "py"
            and "(" not in item.name
            and (item.name == "ffmpeg_py_gui" or item.name.startswith("ffmpeg_py_gui."))
        ):
